...
const navigate = useNavigate();
  const steps = [
    ...
    { id: 3, title: 'Plans & Site Details', description: 'Excavation plans and site info' },
  ...
  const handleSubmit = async (e) => {
    ...
    // Simulate API call
    setTimeout(() => {
      setSubmitStatus({ type: 'success', message: 'Excavation permit application submitted!' });
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      ...
      case 2:
        return (
          <div className="space-y-6">
            <h3 className="text-xl font-semibold mb-4">Engineer & Contractor</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block mb-2 font-medium">Civil/Geotechnical Engineer Name *</label>
      ...
      case 3:
        return (
          <div className="space-y-6">
            <h3 className="text-xl font-semibold mb-4">Plans & Site Details</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block mb-2 font-medium">Excavation Depth & Profile *</label>
                <input type="text" name="excavation_depth" value={formData.excavation_depth} onChange={handleChange} className="w-full p-3 border rounded-lg" required />
              </div>
              <div>
                <label className="block mb-2 font-medium">Retaining Wall Design</label>
                <input type="checkbox" name="retaining_wall" checked={formData.retaining_wall} onChange={handleChange} className="mr-2" />
                <span>Included</span>
              </div>
              <div>
                <label className="block mb-2 font-medium">Shoring / Slope Protection System</label>
                <input type="checkbox" name="shoring_protection" checked={formData.shoring_protection} onChange={handleChange} className="mr-2" />
                <span>Included</span>
              </div>
              <div>
                <label className="block mb-2 font-medium">Site Plan with Neighboring Structures *</label>
              ...
              <div>
                <label className="block mb-2 font-medium">Geotechnical Investigation Report</label>
                <input type="checkbox" name="geotech_report" checked={formData.geotech_report} onChange={handleChange} className="mr-2" />
                <span>Included</span>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="mx-1 mt-1 p-6 rounded-lg min-h-screen" style={{ background: '#fbfbfb', color: '#222' }}>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl md:text-4xl font-bold" style={{ color: '#4a90e2' }}>Excavation/Grading Permit Application</h1>

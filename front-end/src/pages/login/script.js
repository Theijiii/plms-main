...
async function handleLoginSubmit(event) {
    ...
    try {
        const response = await makeAPICall('/login/login.php', { email, password, action: 'login' });

         if (response.success && response.data && response.data.requireOtp) {
            openOtpModal();
        } else {
 ...
 document.addEventListener('DOMContentLoaded', () => {
    ...
    if (otpForm) {
        otpForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            const otp = collectOtpCode();

            try {
                 const response = await makeAPICall('/login/login.php', { otp, action: 'verifyOtp' });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (cancelOtp) cancelOtp.addEventListener('click', closeOtpModal);
     if (resend) resend.addEventListener('click', async () => {
         try {
             resend.disabled = true;
             const resp = await makeAPICall('/login/login.php', { action: 'resendOtp' });
             if (resp.success) {
                 showNotification(resp.message || 'A new OTP has been sent to your email.', 'info');
                 startOtpTimer(180);
             } else {
                 showNotification(resp.message || 'Unable to resend OTP.', 'error');
     ...
     if (otpForm) otpForm.addEventListener('submit', (e) => {
        ...
        error.classList.add('hidden');
         // keep logic here minimal; actual verification handled in the other listener above
    });
    if (modal) {

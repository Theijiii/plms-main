...
export default function MechanicalPermit() {
  ...
  const steps = [
    ...
    { id: 3, title: 'Plans & Equipment', description: 'Mechanical plans and equipment info' },
  ...
  const handleSubmit = async (e) => {
    ...
    // Simulate API call
    setTimeout(() => {
      setSubmitStatus({ type: 'success', message: 'Mechanical permit application submitted!' });
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      ...
      case 2:
        return (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block mb-2 font-medium">Professional Mechanical Engineer Name *</label>
      ...
      case 3:
        return (
          <div className="space-y-6">
            <h3 className="text-xl font-semibold mb-4">Plans & Equipment</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block mb-2 font-medium">Equipment List *</label>
                <textarea name="equipment_list" value={formData.equipment_list} onChange={handleChange} rows="3" className="w-full p-3 border rounded-lg" required />
              </div>
              <div>
                <label className="block mb-2 font-medium">Capacity / Rating *</label>
                <input type="text" name="capacity" value={formData.capacity} onChange={handleChange} className="w-full p-3 border rounded-lg" required />
              </div>
              <div>
                <label className="block mb-2 font-medium">Schematics / Diagrams *</label>
                <input type="checkbox" name="schematics" checked={formData.schematics} onChange={handleChange} className="mr-2" />
                <span>Included</span>
              </div>
              <div>
      ...
      case 4:
        return (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block mb-2 font-medium">PRC ID, PTR & Tax Receipt *</label>
  ...
  return (
    <div className="mx-1 mt-1 p-6 rounded-lg min-h-screen" style={{ background: '#fbfbfb', color: '#222' }}>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl md:text-4xl font-bold" style={{ color: '#4a90e2' }}>Mechanical Permit Application</h1>
